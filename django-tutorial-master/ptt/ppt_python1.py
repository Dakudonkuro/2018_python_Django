...
def parseGos(link):
    ...
    if(link!="https://www.ptt.cc/bbs/Gossiping/M.1510415718.A.D77.html"):
        if(title!="no title"):
            cutword(main_content,title,jiebatf,link)
    #print('ptitle',title)
    #print('plink',link)
...
def cutword(main_content,title,jiebatf,link):
    cutmain=0
    cutmaindata = {}
    cutsmaindata = {}
    ...
    pointdata = round((keyloop/len(cutsmaindata))*100,2)
    ...
    onemaintidf = jieba.analyse.extract_tags(main_content,10)
